...
bool GraphSprite::init(Graph & graph)
{
    ...
    if (strcmp(graph.name.substr(0, 3).c_str(), "map") != 0)
    {
        ...
        if (!UserDefault::getInstance()->getBoolForKey(ss.str().c_str(), false) && !UserDefault::getInstance()->getBoolForKey(oldUser.c_str(),false))
        {
            Sprite* lSprite = Sprite::create("other/new.png");
            lSprite->setAnchorPoint(Vec2(1, 1));
            
            auto world = bg->convertToWorldSpace(Vec2(bg->getContentSize()));
            lSprite->setPosition(convertToNodeSpace(world) + Vec2(-11, -11));
            addChild(lSprite, 10);
        }
    }
...
void GraphSprite::imageLoadedCallback()
{
    ...
    if (lSprite)
    {
        auto color = LayerColor::create(Color4B(m_gGraphInfo.bgColor), 420, 420);
        color->setPosition(Vec2(getContentSize()/2) - Vec2(color->getContentSize()/2));
        addChild(color);
