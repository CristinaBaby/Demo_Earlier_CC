...
LayoutEx * quickLayoutColor(const string& name, Widget* root, const Widget::ccWidgetTouchCallback &callback)
{
    auto layoutEx = quickLayoutColor(name, root, callback, nullptr);
    layoutEx->setPresent(layoutEx);
    return layoutEx;
}

LayoutEx * quickLayoutColor(const string& name, Widget* root, const Widget::ccWidgetTouchCallback &callback, Widget * present)
{
    //复制所有属性
    Layout * original = (Layout*)Helper::seekNodeByName(root, name);
    LayoutEx * layoutEx = LayoutEx::create();
...
Text * quickText(const string& name, Widget* root, const Widget::ccWidgetTouchCallback &callback)
{
    ...
    underline->drawSegment(text->getPosition() - text->getContentSize()/2,
                           Vec2(text->getPosition().x + text->getContentSize().width/2, text->getPosition().y - text->getContentSize().height/2), 1, Color4F(text->getColor()));
