...
ServerSprite::ServerSprite()
{
    ...
    NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(ServerSprite::LoadMapSprite), st_datamanager_dynamicLoadTexture, NULL);
}
...
//        this->addChild(lSprite);
        ...
        if (!m_pMenuItem)
        {
            ...
            MenuItemImage* lMenuItem = MenuItemImage::create("heart1.png", "heart1.png", CC_CALLBACK_1(ServerSprite::reponseMenu, this));
            lMenuItem->ignoreAnchorPointForPosition(false);
            lMenuItem->setAnchorPoint(Vec2(0, 1));
            lMenuItem->setPosition(Vec2(ScreenAdapterHelper::getValue(20), ScreenAdapterHelper::getValue(0)));
            ...
            label->setPosition(Vec2(lMenuItem->getContentSize().width * 1.2, lMenuItem->getContentSize().height/2));
...
void ServerSprite::removeTextureCache()
{
    ...
    for (int i = 0; i < DataManager::getInstance()->m_vNeedLoadImage.size(); ++i)
    {
        string str = DataManager::getInstance()->m_vNeedLoadImage.at(i);
        if (str == this->getImageFullPath())
        {
            std::vector<std::string>::iterator iter = DataManager::getInstance()->m_vNeedLoadImage.begin() + i;
            DataManager::getInstance()->m_vNeedLoadImage.erase(iter);
            break;
        }
    }
...
void ServerSprite::waitDownLoadSuc(Ref* ref)
{
    ...
    if (FileUtils::getInstance()->isFileExist(this->getImageFullPath()) && m_bIsDownloadFinish == false)
    {
        ...
        Label* label = Label::createWithTTF("", st_font_AR, ScreenAdapterHelper::getValue(50));
        label->ignoreAnchorPointForPosition(false);
        label->setAnchorPoint(Vec2(0, 0.5));
        label->setPosition(Vec2(lMenuItem->getContentSize().width * 1.2, lMenuItem->getContentSize().height/2));
        lMenuItem->addChild(label);
