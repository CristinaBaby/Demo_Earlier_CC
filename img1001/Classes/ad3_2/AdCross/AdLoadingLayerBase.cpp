...
void AdLoadingLayerBase::loadAd(bool abIscross)
{
    ...
    this->updateLayer(eInterstitial_Normal_Type);
...
void AdLoadingLayerBase::loadReward(bool abIscross)
{
    if(abIscross){
        return;
    }
    _adLoadType = kTypeRewardedAds;
    _isRequestingLoadAd = false;
    _adShowing = false;
    _bLoadingCanRemove = false;
    _adLoadDone = false;
    ...
    this->updateLayer(eInterstitial_Rward_Type);
    ...
    {
        bool ret = AdsManager::getInstance()->showAds(kTypeRewardedAds);
        if(!ret)
        {
            _isRequestingLoadAd = true;
        }
...
void AdLoadingLayerBase::onRewardedCollapsed(std::string unitId, int amount, bool bIsSkipped)
{
    _taskDone();
