      ...
      //  CCLOG("Touch %d %d Color %d %d %d %d", col, row, _startColor.r, _startColor.g, _startColor.b, _startColor.a);
        ...
        if (lColor.a > 0)
        {
            ...
            if(lResBool)
            {
                ...
                Color4B color = Color4B(DataManager::getInstance()->m_cSelectedColor.R, DataManager::getInstance()->m_cSelectedColor.G, DataManager::getInstance()->m_cSelectedColor.B, 255);
                
                this->ScanLineSeedFill(col, row, color, Color4B(0, 0, 0, 255));
...
//                    }
                ...
                DataManager::getInstance()->m_bCurrentImageIsUsed = true;
                DataManager::getInstance()->m_bEnableRateUs = true;
                DataManager::getInstance()->m_bIsAddMyAtrworks = true;
                DataManager::m_iCurrentClickNumber++;
        ...
        {
            GameLayer* lGameLayer=(GameLayer*)this->getParent()->getParent();
            if(pTouch->getLocation().y<ScreenAdapterHelper::getRealDesignSize().height-ScreenAdapterHelper::getValue(100))
            {
                
                DotColorData lDotColorData;
                
                Color3B mColor = DataManager::getInstance()->getSaveBackColor(DataManager::getInstance()->m_pCurrentImage.imageName, DataManager::getInstance()->m_pCurrentImage.ID);
                
              //  Color3B mColor = DM_getSaveBackColor;
...
//    this->getEventDispatcher()->addEventListenerWithSceneGraphPriority(listener, this);
    this->getEventDispatcher()->addEventListenerWithFixedPriority(listener, 2);
    return true;
}
...
void BoundaryFillNode::undo()
{
   if(_dotColorDatas.size()>0)
   {
       ...
       Color4B lColor = ScribbleUtil::getColor(_lineImage, pos);
       log("click dot alpha = %d",lColor.a);
       if (lColor.a > 0) {
