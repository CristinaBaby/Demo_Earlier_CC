...
cocos2d::Scene* ShareScene::scene(int pIndex,GameScene* pGameScene)
{
    ...
    if (pRet && pRet->init(pIndex, pGameScene, lScene))
    {
...
bool ShareScene::init(int pIndex, GameScene* pGameScene, Scene * parent)
{
...
//    this->addChild(layerColor);
    ...
    if (DataManager::m_bIspainted)
    {
        DataManager::m_bIspainted = false;
        xAdLoading->show(ADS_TYPE::kTypeInterstitialAds, parent);
    }
