...
ChooseMapLayer::ChooseMapLayer()
{
    ...
    m_pDotSprite = NULL;
    m_pPageLabel = NULL;
    m_pDownloadLabel = NULL;
    m_iDownloadIndex = 0;
    
    NotificationCenter::getInstance()->addObserver(this,callfuncO_selector(ChooseMapLayer::waitStartDownload),st_begin_download_image,NULL);
...
bool ChooseMapLayer::init()
{
    ...
    if (ImageDownload::_isDownloading)
    {
        ScreenAdapterHelper::setPosition(label, Vec2(264, 160));
        
        Label* label2 = Label::createWithSystemFont("(downloading   )", "Arial", ScreenAdapterHelper::getValue(40));
        ScreenAdapterHelper::setPosition(label2, Vec2(354, 160));
        label2->ignoreAnchorPointForPosition(false);
        label2->setAnchorPoint(Vec2(0, 0.5));
        this->addChild(label2);
        m_pDownloadLabel = label2;
        
        this->schedule(schedule_selector(ChooseMapLayer::update), 1.0f);
    }
...
void ChooseMapLayer::tableCellTouched(TableView* table, TableViewCell* cell,Touch* pTouch)
{
    ...
    else
    {
        ...
        UserDefault::getInstance()->setIntegerForKey(st_rateUs_count, ++rateUs);
...
void ChooseMapLayer::waitStartDownload(Ref* ref)
{
    
}

string down[4] = {"(downloading   )", "(downloading.  )", "(downloading.. )", "(downloading...)"};

void ChooseMapLayer::update(float dt)
{
    if (m_pDownloadLabel)
    {
        string str = down[m_iDownloadIndex];
        m_pDownloadLabel->setString(str);
        m_iDownloadIndex ++;
        
        if (m_iDownloadIndex > 3)
        {
            m_iDownloadIndex = 0;
        }
    }
...
void ChooseMapLayer::waitDownloadFinish(Ref* ref)
{
