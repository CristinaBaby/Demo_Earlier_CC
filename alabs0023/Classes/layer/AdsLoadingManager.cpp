...
void AdsLoadingManager::show(ADS_TYPE type, Scene * scene)
{
    CCASSERT(scene != nullptr, "error");
    show(type);
    _runningScene = scene;
}
...
void AdsLoadingManager::update(float dt)
{
    switch (_state) {
        ...
        {
            if ((_adType == ADS_TYPE::kTypeCrosspromoAds && _runningScene != Director::getInstance()->getRunningScene()) ||
                (_adType == ADS_TYPE::kTypeInterstitialAds &&
                 (_runningScene == Director::getInstance()->getRunningScene() || _runningScene == nullptr))) {
