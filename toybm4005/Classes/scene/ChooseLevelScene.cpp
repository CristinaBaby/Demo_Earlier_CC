...
#define st_orgin_version_image_num 10
...
bool ChooseLevelScene::init(int pIndex)
{
    ...
    LayerColor* layerColor=LayerColor::create(Color4B(254, 130, 168, 255));
...
//    ScreenAdapterHelper::setPosition(label, Vec2(384, 900),true,false,false,false);
    ...
    Sprite* text = Sprite::create("ChooseScene_text.png");
    ScreenAdapterHelper::setPosition(text, Vec2(384, 900),true,false,false,false);
    this->addChild(text);
    text->setScale(1.2f);
...
void ChooseLevelScene::reponseTouchEvent(ImageBox* pImageBox)
{
    ...
    UserDefault::getInstance()->setBoolForKey(lImageBox->getImageName().c_str(), false);
