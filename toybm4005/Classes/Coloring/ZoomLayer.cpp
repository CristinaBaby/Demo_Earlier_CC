...
//        GameScene* lGameScene=(GameScene*)this->getParent()->getParent();
//        lGameScene->getGameUILayer()->removeTutorial();
//        
        _contentNode->setUserData((void *)0x1);
        
        auto point1 = _contentNode->getParent()->convertToNodeSpace(pTouches[0]->getLocation());
        _startPoint=point1;
...
#define RATIO 1.0f
            ...
            Touch* lTouch=touches[0];
            
            if(lTouch->getLocation().y<ScreenAdapterHelper::getValue(400) ||  lTouch->getLocation().y>ScreenAdapterHelper::getRealDesignSize().height-ScreenAdapterHelper::getValue(120))
            {
                return;
            }
            
            Vec2 point1 = _contentNode->getParent()->convertToNodeSpace(touches[0]->getLocation());
            float lDistance=point1.distance(_startPoint);
            
            //CCLOG("diatance=%f",lDistance);
            
            if(lDistance>=10) //一些android 点上去就会移动像素 所以这里增加一个判断
            {
                _contentNode->setUserData((void *)0x0);
            }
